...
import { BrainCircuit, Globe, Smartphone, Upload, Loader2 } from 'lucide-react';
...
export function DomainTracks() {
  ...
  const startSession = async () => {
    try {
      ...
      // Upload resume if user selected one
      if (resumeFile) {
      ...
      const domainStr = selectedTags.length > 0 
        ? `${currentTrack?.title} (Focus: ${selectedTags.join(', ')})`
        : currentTrack?.title || 'General';
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-6">
            ...
            <Button
              ...
              {isStarting ? 'Starting...' : 'Start Domain Interview'}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
